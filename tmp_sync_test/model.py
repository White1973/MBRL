...
class VLLMModelInterface(BaseModelInterface):
    ...
    def __init__(self, config: VLLMModelConfig):
        ...
        # If this is a multimodal model, try to handle according to the installed vLLM version.
        if self.is_multimodal:
            logger.info(
                "Detected multimodal model: %s (vLLM %s)",
                self.model_name,
                self.vllm_version,
            )

            # Older VAGEN code passed legacy VLM kwargs through EngineArgs. vLLM 0.8.x
            # handles multimodal models directly from the per-prompt TextPrompt schema,
            # so the legacy kwargs now raise during construction.
            try:
                from vllm.engine.arg_utils import EngineArgs

                supported_params = set(EngineArgs.__init__.__code__.co_varnames)
                wants_legacy_vlm_args = "image_input_type" in supported_params
            except Exception:
                wants_legacy_vlm_args = False

            if wants_legacy_vlm_args:
                extra_vlm_args = {}
                ...
                try:
                    ...
                    model_kwargs["engine_args"] = EngineArgs(**model_kwargs, **extra_vlm_args)
                    logger.info("Using legacy multimodal EngineArgs path")
                except Exception as e:
                    logger.warning("Legacy EngineArgs path failed: %s", e)
            else:
                logger.info("Skipping legacy multimodal EngineArgs; using prompt-level multimodal inputs")
    ...
    def generate(self, prompts: List[Any], **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            if self.is_multimodal and any(img is not None for img in image_inputs):
                ...
                try:
                    prompt_objects: List[TextPrompt] = []
                    for text, images in zip(formatted_prompts, image_inputs):
                        prompt_obj: TextPrompt = {"prompt": text}
                        if images is not None:
                            prompt_obj["multi_modal_data"] = {"image": images}
                        prompt_objects.append(prompt_obj)
                    outputs = self.model.generate(prompt_objects, sampling_params)
                except Exception as e:
                    logger.warning(f"Prompt-level multimodal generation failed: {e}. Falling back to text-only generation.")
