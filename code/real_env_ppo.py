        ...
        *,
        ...
        for t in text:
            ...
            while self.IMAGE_PAD in result and grid_thw is not None and img_idx < grid_thw.shape[0]:
                g = grid_thw[img_idx]
                n_tokens = int(g[0].item() * g[1].item() * g[2].item())
                result = result.replace(self.IMAGE_PAD, self.IMAGE_PAD * n_tokens, 1)
                img_idx += 1
...
class RGBFreeformPolicy(nn.Module):
    ...
    def act(
        ...
        *,
        ...
        prompt_batch = self._prepare_prompt_batch(observations, env_ids, prompt_texts=prompt_texts)
        ...
        actions, parse_success, parsed_texts = self.parser.parse_with_success(texts, env_ids)
    ...
    def evaluate_actions(
        ...
        *,
        ...
        values = self._value_from_prompt_batch(
            self._prepare_prompt_batch(observations, env_ids, prompt_texts=prompt_texts)
        )
        token_log_probs, token_entropies = self.response_token_stats(
            observations,
            env_ids,
    ...
    def _prepare_prompt_batch(
        ...
        *,
        ...
        resolved_prompt_texts: list[str] = []
        ...
        batch = self.processor(
            images=[self._to_pil_image(obs) for obs in observations],
            text=texts,
            padding=True,
            return_tensors="pt",
        )
    ...
    def _build_prompts(self) -> list[str]:
        prompts: list[str] = []
        for _env_name, action_texts in zip(self.env_ids, self.action_texts_by_env, strict=True):
            action_list = ", ".join(action_texts)
            prompts.append(
                "[Initial Observation]\n"
                "Look at the image and decide the single best next action.\n"
                f"Valid actions: {action_list}.\n"
                "First reason briefly about the relative positions of the player, box, and target. "
                "Then output exactly one action.\n"
                f"Use the format <think>...</think>{self.ff_cfg.answer_open_tag}ACTION{self.ff_cfg.answer_close_tag}."
            )
        return prompts
    ...
    def _build_system_prompts(self) -> list[str]:
        ...
        for action_texts in self.action_texts_by_env:
            ...
            prompts.append(
                ...
                "Goal: push the box onto the target.\n"
                "Rules:\n"
                "1. You may move one square at a time.\n"
                "2. You can push a box only by moving into it from an adjacent square.\n"
                "3. You cannot pull boxes.\n"
                "4. Walls block movement.\n"
                f"Valid actions are: {action_list}.\n"
                "Example 1:\n"
                f"<think>The box is below me and the target is further below. I should move Down to approach the box and push it toward the target.</think>{self.ff_cfg.answer_open_tag}Down{self.ff_cfg.answer_close_tag}\n"
                "Example 2:\n"
                f"<think>The box is to my right and the target is further right. I should move Right to line up for a push.</think>{self.ff_cfg.answer_open_tag}Right{self.ff_cfg.answer_close_tag}\n"
                "Example 3:\n"
                f"<think>The box is above me and the target is above the box. I should move Up to reach the box and push it upward.</think>{self.ff_cfg.answer_open_tag}Up{self.ff_cfg.answer_close_tag}"
            )
        return prompts
...
class RealEnvPPOTrainer:
    ...
    def collect_rollout(self, *, update: int) -> RealEnvRolloutBatch:
        ...
        for episode_idx in range(num_episodes):
            ...
            try:
                ...
                step_idx = 0
                previous_action_text: str | None = None
                while not done and step_idx < max_steps:
                    obs_list = [obs]
                    env_tensor = torch.tensor([env_id], device=self.device, dtype=torch.long)
                    step_prompt = self._format_step_prompt(previous_action_text)
                    step = self.policy.act(
                        obs_list,
                        env_tensor,
                        deterministic=False,
                        prompt_texts=[step_prompt],
                    )
                    env_action = self._to_env_action(adapter, int(step.actions[0].item()))
                    ...
                    previous_action_text = step.parsed_texts[0] if bool(step.parse_success[0].item()) else None
                    obs = next_obs
                    step_idx += 1
        ...
        for episode_idx in range(num_episodes):
            ...
            try:
                ...
                while not done and ep_len < max_steps:
                    step_prompt = self._format_step_prompt(previous_action_text)
                    step = self.policy.act([obs], env_tensor, deterministic=True, prompt_texts=[step_prompt])
                    env_action = self._to_env_action(adapter, int(step.actions[0].item()))
    ...
    def _format_step_prompt(self, previous_action_text: str | None) -> str:
        if previous_action_text is None:
            return self.policy.prompts_by_env[0]
        return (
            f"After your previous answer, the extracted valid action was {previous_action_text}.\n"
            "After that action, here is the new observation.\n"
            "Decide your next single action.\n"
            "First reason briefly about the player, box, and target, then answer with exactly one action.\n"
            f"Use the format <think>...</think>{self.config.phase2.freeform_policy.answer_open_tag}ACTION"
            f"{self.config.phase2.freeform_policy.answer_close_tag}."
        )
