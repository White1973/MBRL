...
class SokobanEnv:
    ...
    def __init__(
        ...
        min_solution_steps: int | None = None,
        max_solution_steps: int | None = None,
        max_reset_attempts: int = 128,
    ) -> None:
        ...
        self._min_solution_steps = min_solution_steps
        self._max_solution_steps = max_solution_steps
    ...
    def reset(self, *, seed: int | None = None) -> np.ndarray:
        ...
        base_seed = int(s) if s is not None else None
        for attempt_idx in range(self._max_reset_attempts):
            if base_seed is not None:
                np.random.seed(base_seed + attempt_idx)
            obs = self._env.reset()
            state = _extract_sokoban_state(self)
            if self._matches_solution_filter(state):
        ...
        raise RuntimeError(
            "Failed to sample a Sokoban puzzle matching the requested solution-length "
            f"filter after {self._max_reset_attempts} reset attempts. "
            f"Requested range: [{self._min_solution_steps}, {self._max_solution_steps}]"
        )
    ...
    def _matches_solution_filter(
        ...
        if self._min_solution_steps is None and self._max_solution_steps is None:
            return True
        if state is None:
            return False
        player, boxes, targets, walls = state
        ...
        if self._min_solution_steps is not None and solution_len < self._min_solution_steps:
            return False
        if self._max_solution_steps is not None and solution_len > self._max_solution_steps:
            return False
        return True
...
class SokobanAdapter:
    ...
    def make_env(self, seed: int | None = None) -> EnvProtocol:
        return SokobanEnv(
            ...
            min_solution_steps=self._min_solution_steps,
            max_solution_steps=self._max_solution_steps,
            max_reset_attempts=self._max_reset_attempts,
        )
    ...
    def to_env_action(self, model_action: Any) -> int:
        """Map model action (0-3) back to Sokoban env action (1-4)."""
        return int(model_action) + 1
