...
def _normalize_actions(action_content: str, action_sep: str, max_actions: int) -> tuple[str, List[str]]:
    normalized = action_content.strip()
    if not normalized:
        return "", []

    actions = [action.strip() for action in normalized.split(action_sep) if action.strip()]
    if not actions:
        actions = [match.group(1).capitalize() for match in ACTION_PATTERN.finditer(normalized)]

    canonical_actions: List[str] = []
    for action in actions:
        lowered = action.strip().lower()
        if lowered in {"up", "down", "left", "right"}:
            canonical_actions.append(lowered.capitalize())

    if len(canonical_actions) > max_actions:
        canonical_actions = canonical_actions[:max_actions]
    normalized_content = action_sep.join(canonical_actions)
    return normalized_content, canonical_actions
...
def parse_freethink(response: str, special_token_list=None, action_sep=',', max_actions=3) -> Dict:
    ...
    if not strict_match:
        ...
        fallback_match = re.search(r"<answer>(.*?)</answer>", response, re.DOTALL)
        if fallback_match:
            action_content = fallback_match.group(1)
            if special_token_list is not None:
                for special_token in special_token_list:
                    action_content = action_content.replace(special_token, "").strip()
            action_content, actions = _normalize_actions(action_content, action_sep, max_actions)
        else:
            answer_label_match = re.search(r"Answer\s*:\s*(.*)", response, re.IGNORECASE | re.DOTALL)
            if answer_label_match:
                action_content = answer_label_match.group(1)
                if special_token_list is not None:
                    for special_token in special_token_list:
                        action_content = action_content.replace(special_token, "").strip()
                action_content, actions = _normalize_actions(action_content, action_sep, max_actions)
...
def parse_no_think(response: str, special_token_list=None, action_sep=',', max_actions=3) -> Dict:
    ...
    if not strict_match:
        ...
        if special_token_list is not None:
            for special_token in special_token_list:
                fallback_text = fallback_text.replace(special_token, "").strip()
        action_content, actions = _normalize_actions(fallback_text, action_sep, max_actions)
    else:
